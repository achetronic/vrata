...
func mapInlineTools(prefix string, rules []agentic.AccessRule, policyNs, trustDomain string) *MappedXAccessPolicy {
	...
	for _, rule := range rules {
		sourceCEL := sourceToCEL(rule.Source, policyNs, trustDomain)
		if sourceCEL == "" {
			continue
		}
...
// sourceToCEL converts a Source to a CEL expression that matches the caller identity.
func sourceToCEL(src agentic.Source, policyNs, trustDomain string) string {
	switch src.Type {
	case "SPIFFE":
		if src.SPIFFE == nil {
			return ""
