...
Generated: 2026-03-18 (rev 5 — after full source audit, all critical/high bugs fixed)
...
|---|---|---|
| Access log file handle never closed | Low | Process-lifetime resource, acceptable |
| Access log concurrent writes unsynchronized | Low | Interleaved lines possible under extreme concurrency |
| gRPC connections per-request (extproc, extauthz) | Medium | No connection pooling. Works, but inefficient at scale |
...
| Rate limit XFF spoofable | Low | Trusts X-Forwarded-For from any client |
| JWT exp claim not required | Low | Tokens without exp valid indefinitely |
| SSE no heartbeat/keepalive | Low | Dead connections detected by client timeout only |
