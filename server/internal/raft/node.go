...
// NewNode creates and starts a Raft node. httpAddr is the HTTP address of
// this node (e.g. ":8080"), used to derive the leader's HTTP address when
// forwarding writes.
func NewNode(ctx context.Context, cfg *config.ClusterConfig, store *boltstore.Store, logger *slog.Logger, httpAddr string) (*Node, error) {
	fsm := NewFSM(store)
	...
	if err := os.MkdirAll(cfg.DataDir, 0755); err != nil {
		return nil, fmt.Errorf("creating raft data dir %q: %w", cfg.DataDir, err)
	}
	...
	snapshotStore, err := raft.NewFileSnapshotStore(cfg.DataDir, snapshotRetain, newSlogWriter(logger))
	if err != nil {
		logStore.Close()
		stableStore.Close()
